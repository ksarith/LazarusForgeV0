...
**Version 4.6 — 2026-07-02. Two items closed from v4.5: Security_Protocols.md's SEC-007 sidecar split reconciled, and the Grok/Gemini G3/G6 gate-maturity dispute resolved via a new Forge_Audit_Kit.md v1.5 clarification.**
...
- **SEC-007 sidecar split reconciled** — `Admin/Security_Protocols.md`'s own sidecar entry is now split into SEC-007a/SEC-007b (file bumped to v0.7), matching the split registered here since v4.4. PAT-001/PAT-002 in the EDL registry now point at entries that actually exist in the split form. Downstream refs corrected in the owning file (Trust Boundary Declaration, Relationship section, Status footer).
- **Grok/Gemini G3/G6 dispute resolved** — logged as **SEC-DS-001** in `Admin/Security_Protocols.md` Active Disputes (previously an unstructured prose flag; `Admin/Auditor_Protocols.md` §Dispute Handling Protocol requires disputes be tracked explicitly, not left informal). Resolved via a new **Gate Scope vs. Promotion Readiness** clarification added to `Admin/Forge_Audit_Kit.md` v1.5, sourced from `Admin/Auditor_Protocols.md` §Specification Promotion Rules: "all six gates pass" and "open unknowns are non-blocking" are separate, independently required promotion conditions — gates test a document's own execution quality, not whether upstream unknowns are resolved. Gemini's original G3/G6 scoring was structurally correct; Grok's underlying concern about unresolved SEC-007a/b etc. is valid but belongs to promotion-readiness tracking, which Security_Protocols.md already carries separately in its Status footer.
- **Process note:** the previous turn's edit that first attempted to record the SEC-007 split closure did so by mutating v4.5's "What it means" text in place, after the user had already confirmed v4.5 was committed. That was incorrect — committed version text should not be revised without a new version bump (the same append-only discipline `Admin/Repository_Integrity_Protocol.md`'s Lessons Learned already documents). Reverted; v4.5's original text is preserved unchanged in the Audit Trail below, and this v4.6 entry is the correct place for both closures.
- `Admin/Forge_Audit_Kit.md` bumped to v1.5 in the same pass — its `Unknowns.md` derivation string was stale at v4.0 (three missed maintenance-trigger updates); corrected. Critical watch summary still needs a full refresh against v4.0→v4.6 changes — flagged as FAK-007, not done this session.
