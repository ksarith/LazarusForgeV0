 ...
 * **The Scrubber Prerequisite:** If any cell temperature probe registers greater than 55°C, the main battery disconnect relay must open immediately via hardware trip, and the Air_Scrubber.md variant 0 positive-pressure loop must automatically lock into max-flow mode to purge hydrofluoric acid gas byproducts away from human operators.
...
|---|---|---|---|---|
| **Logic / Watchdog** | Less than 15 W baseline | \pm 1\% | Primary TEG Bank / Small Isolated Lead-Acid Buffer | Maintain state indefinitely; broadcast keep-alive telemetry. |
| **Mechanical Milling** | 1.5 kW peak | \pm 5\% | Active Hydro-Engine / Main Battery Bank (\text{SoH} > 70\%) | Cycle spindle speed down 50%; halt axis travel stepper logic. |
| **Nominal Mode** | 15–40 kW nominal range | \pm 5\% | Grid Power / Scaled Generator Blocks + Air Scrubber | Throttle material feed rates; pause secondary processing axes. |
| **Thermal Melt (G5)** | 8.0 kW burst | \pm 10\% | Direct Generator / Dedicated Biomass Syngas Loop | Trigger immediate safety clamp; dump molten charge to dry safe crucible. |
## Metrics (Falsifiable)
 ...
 * **Description:** Actual consumption of the physical bootstrap hardware remains an estimate based on industrial analogs.
 * **Resolution Path:** Power Demand stub bound to concrete operational mode envelopes (Logic, Milling, Nominal, Thermal Melt). Replace analog values with real shunt-resistor telemetry as hardware validation completes.
 ...
 * **Resolution Path:** Enforced the 35°C thermal interlock threshold and 22% maximum compressor energy rule. Next step: capture gas-flow meter data against input feedstock mass vectors during initial operational loops.
### EV-003 — Salvaged battery thermal containment and ventilation strategy undefined
 * **Status:** In Progress | **Risk:** High | **Priority:** Critical
 * **Blocking:** Yes (before enclosed battery bank commissioning).
 * **Description:** Unknown state-of-health battery modules present catastrophic runaway vectors.
 * **Resolution Path:** Imposed physical isolation rules, locked 3.0V per cell hard-guards, and mapped the Air Scrubber Max-Flow auto-trigger coupling. Move to resolved once the external fire-rated enclosure physical build passes cold safety audit.
## Resolution Log
 ...
 * **2026-05-31:** Restructured document to achieve Spec Gate 1 baseline. Integrated Tar Minimization interlock and the 35°C/22% Biogas efficiency threshold into the core Energy Sources layer. Bound EV-001 to the specific hardware envelope profile table. Fully codified the physical isolation boundaries and automated air scrubber max-flow overrides for EV-003. Cleaned double-hash tracking headers.
