...
**Version 4.5 — 2026-07-02. External Design Lineage (EDL) registry added to Security_Protocols.md — PAT-001 through PAT-004 anchor SEC-007a, SEC-007b, SEC-005, and SEC-011. Cross-refs added here. A proposal to canonize EDL as a repository-wide constitutional gate was not adopted — flagged for Governance_Migration_Protocol.md routing instead.**
...
- **External Design Lineage (EDL) registry added** to `Admin/Security_Protocols.md`, positioned after the Trust Boundary Declaration. Logs external engineering patterns (industry standards, cross-domain precedent) as evidence weighed against Forge doctrine, not authority — ties to EF-0.0/EF-0.1. Four entries seeded:
  - PAT-001 → SEC-007a (NIST SP 800-57/FIPS 140-3 root-of-trust models; Modify; Decision Drafted)
  - PAT-002 → SEC-007b (TCG/IETF measured boot; Modify; Decision Drafted)
  - PAT-003 → SEC-005 (NIST SP 800-90 attestation; Modify; Decision Drafted)
  - PAT-004 → SEC-011 (NIST/IETF PQC migration; Observe with extension; Researching; 2-cycle Expiry Watch)
- Cross-refs added to SEC-005, SEC-007a, SEC-007b, SEC-011 rows above (`— Design Lineage: PAT-00X`).
- **Correction applied:** an earlier draft of PAT-004's rationale mistakenly referenced SEC-012 for a proposed symmetric-key fallback tree. SEC-012 remains scoped to asymmetric-crypto overhead on salvaged silicon (registered v4.4) — unrelated. A symmetric-fallback unknown, if logged, requires a new ID (proposed SEC-013), not registered this pass.
- **Not adopted this session:** a companion proposal (GOV-MAND-009) to canonize EDL as a repository-wide constitutional gate blocking Exploration→Candidate Specification promotion for all files, with automated AUDIT_HARNESS.py enforcement. Flagged as a structural governance change requiring `Admin/Governance_Migration_Protocol.md` two-track ratification rather than session-level multi-agent adoption. EDL registry remains scoped to Security_Protocols.md pending that decision.
- Known drift, unresolved: Security_Protocols.md's own SEC-007 sidecar entry has not yet been split to match the SEC-007a/SEC-007b split registered here in v4.4; PAT-001/PAT-002 are written against the split IDs. Grok/Gemini G3/G6 gate-maturity disagreement (flagged v4.4) also remains unresolved, pending human ruling.
