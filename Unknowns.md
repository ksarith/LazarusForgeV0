...
**Version 4.21 — 2026-07-17. GMP-005 and GMP-009 (`Admin/Governance_Migration_Protocol.md`) marked Resolved and removed from the Governance Migration active index per Size Management Rule 2 — both were one gap (Track A/Track B classified by document location rather than constitutional impact), resolved together by redefining both tracks around impact with a Constitutional Impact Statement checklist. `Governance_Charter.md`'s EDL and GOV-013 drafts formally classified Track A as worked examples; both remain PROPOSED, NOT RATIFIED. GMP-002 explicitly noted as NOT affected — its own resolution path requires `Governance_Migration_Protocol.md` to reach Gate 4 / Provisional Specification maturity (currently 0/6 Spec Gates), unrelated to and unmet regardless of the GMP-005/GMP-009 resolution; a same-day external proposal to close GMP-002 now was not applied for this reason.**
...
| CE-006 | Chlorine gas containment doctrine undefined for on-site chlor-alkali acid synthesis — cross-ref `Challenges/Closed_Loop_Feedstock.md` CLF-004 (candidate pathway this blocks) and `Admin/Ethical_Constraints.md` §Toxic and Hazardous Material Handling | `Architecture/Chemistry.md` | Open | — | Critical |
...
| PC-005 | `Challenges/Closed_Loop_Feedstock.md` not registered in `Routing.md`, `Discovery.md`, or `Admin/AUDIT_HARNESS.py` | Closed_Loop_Feedstock.md | Open (possibly stale — file appears present in all three as of 2026-07-12; not independently re-verified this pass, flag for confirmation before assuming Resolved) | — | Major |
| PC-006 | Ethical Anchor field used a non-canonical backticked, `Admin/`-prefixed variant instead of the canonical plain-text string | Biofouling.md, Closed_Loop_Feedstock.md, Critical_Minerals.md, Emergence.md, Energy_Scarcity.md, Planned_Obsolescence.md, Waste.md, Water.md, Support_Raft.md (9 files) | Resolved 2026-07-12 | — | Critical |
